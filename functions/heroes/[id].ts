...
import { html, htmlResponse, fragResponse } from '@lib/html';
...
export const onRequestPatch: PagesFunction = async ({ request, params }) => {
  ...
  return fragResponse(HeroDetails(hero) + Messages(allMessages));
};
...
export const onRequestPost: PagesFunction = async ({ params }) => {
  ...
  if (hero) {
    ...
    return fragResponse(HeroError() + Messages(allMessages));
  }
};
